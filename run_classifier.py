...
## add by bao for text classification
import random
class ClassificationProcessor(DataProcessor):
    """processor the chinese text"""
    def get_train_examples(self, data_dir):
...
####  because the label commit with the train set, we'd better get training data first'
...
#we get labels before do_train 
  ...
  if FLAGS.do_predict:
    predict_examples, test_labels = processor.get_test_examples(FLAGS.data_dir)
    num_actual_predict_examples = len(predict_examples)
    print("the len of test_labels is",len(test_labels))
    if FLAGS.use_tpu:
      # TPU requires a fixed batch size for all batches, therefore the number
      # of examples must be a multiple of the batch size, or else examples
      # will get dropped. So we pad with fake examples which are ignored
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      output_line ="\t".join(str(class_name)+str(i) for i,class_name in enumerate(label_list)) + "\t"+"test_ground_truth"+"\t"+"prediction"'\n'
      ...
      for (i, prediction) in enumerate(result):
        ...
        output_line = "\t".join(
            ...
            for class_probability in probabilities) +"\t"+ "test"+str(test_labels[i])+"\t"+ "predict"+ str(maxindex)+"\n"
        writer.write(output_line)
        num_written_lines += 1
      differ = 0
      for i in range(len(test_labels)):
        if predict_list[i] != test_labels[i]:
            differ += 1
